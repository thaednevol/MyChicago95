...
function centerContainer() {
	...
	var container = document.getElementById("boxSession");
    // Set absolute top to be half of viewport height minus half of container height
    container.style.top = (document.documentElement.clientHeight / 2) - (container.offsetHeight / 2) + 'px';
    // Set absolute left to be half of viewport width minus half of container width
    container.style.left = (document.documentElement.clientWidth / 2) - (container.offsetWidth / 2) + 'px';
...
function centerContainerOnResize() {
	...
	onResize(document.getElementById('boxSession'), centerContainer);
...
function initializeWidgets() {
	...
	// Start up the session widget
    initializeSessionWidget();
}

function initializeSessionWidget() {
	var imgSession = document.getElementById("imgSession");
	var boxSession = document.getElementById('boxSession');
	imgSession.onclick = function(e) {
	if (boxSession.style.display === 'block') {
		boxSession.style.display = 'none';
	} else {
		boxSession.style.display = 'block';
	}
	e.stopPropagation();
	...
	var frmSessions= document.getElementById('frmSessions');
	for (var i=0; i<lightdm.sessions.length; i++) {
		console.log(lightdm.sessions[0].name);
		var label = document.createElement("label");
		label.className = "session";
		
		var x = document.createElement("INPUT");
		x.setAttribute("type", "radio");
		x.setAttribute("id", "frmSession_"+lightdm.sessions[i].key);
		x.setAttribute("value", lightdm.sessions[i].key);
		x.setAttribute("name", lightdm.sessions[i].name);
		label.appendChild(x);
		label.innerHTML += lightdm.sessions[i].name;
		frmSessions.appendChild(label);
	}
...
function initializeHostnameWidget() {
    var imgCloseDialogSession = document.getElementById("imgCloseDialogSession");
	
	var el = document.getElementById("hostname-widget");
    el.innerText = lightdm.hostname;
	el.appendChild(imgCloseDialogSession);
	imgCloseDialogSession.onclick = function(e) {
			console.log("HOLA");
			var boxSession = document.getElementById('boxSession');
            if (boxSession.style.display === 'block') {
        		boxSession.style.display = 'none';
